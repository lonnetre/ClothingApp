...
struct ClothingRowView: View {
    ...
    var body: some View {
        ...
        VStack(spacing: 0) {
            TabView(selection: .constant(categoryPage.selectedItemIndex)) {
                ForEach(0..<categoryPage.items.count, id: \.self) { index in
                    ClothingItemView(
                        item: categoryPage.items[index],
                        isSelected: categoryPage.selectedItemIndex == index,
                        height: CFloat(rowHeight),
